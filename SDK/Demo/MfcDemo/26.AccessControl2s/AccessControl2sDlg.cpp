...
BEGIN_MESSAGE_MAP(CAccessControlDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON1, &CAccessControlDlg::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CAccessControlDlg::OnBnClickedButton2)
END_MESSAGE_MAP()
...
void CAccessControlDlg::OnBnClickedButton1()
{
	if (!Device::GetInstance().GetLoginState())
	{
		MessageBox(ConvertString("Not login yet!"),ConvertString("Prompt"));
	...
	GetDlgItemText(IDC_EDIT1,m_strKey);

	BOOL bret = Device::GetInstance().SetDecodeInfo( m_strKey.GetBuffer(0));
	if (!bret)
	{
		CString csInfo;
		csInfo.Format("%s0x%08x", ConvertString("SetEncodeInfo failed!"), CLIENT_GetLastError());
		MessageBox(csInfo, ConvertString("Prompt"));
	}
	else
	{
		MessageBox("SetEncodeInfo Success!", ConvertString("Prompt"));
	}
	
}


void CAccessControlDlg::OnBnClickedButton2()
{
	if (!Device::GetInstance().GetLoginState())
	{
		MessageBox(ConvertString("Not login yet!"),ConvertString("Prompt"));
	...
	GetDlgItemText(IDC_EDIT1,m_strKey);
	GetDlgItemText(IDC_EDIT3,m_strCardNo);

	char* String = new char[1024];
	memset(String,0,1024);

	BOOL bret = Device::GetInstance().EncryptString(m_strKey.GetBuffer(0),m_strCardNo.GetBuffer(0),String);
	if (!bret)
	{
		CString csInfo;
		csInfo.Format("%s0x%08x", ConvertString("EncryptString failed!"), CLIENT_GetLastError());
		MessageBox(csInfo, ConvertString("Prompt"));
	}
	else
	{
		...
		((CEdit*)GetDlgItem(IDC_EDIT2))->ReplaceSel(CString(String));
		MessageBox("EncryptString Success!", ConvertString("Prompt"));
	}
}
